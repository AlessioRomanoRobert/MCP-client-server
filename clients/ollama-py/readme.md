# Ollama MCP Client (Python)

An async interactive chat client that connects a local [Ollama](https://ollama.com) LLM to an MCP server, enabling **function calling** — the model can invoke MCP tools automatically in response to user messages.
...
```bash
MCP_SERVER_PATH=/path/to/server.js uv run ollama-python-app.py
```

The default model is `mistral:latest`. Change `DEFAULT_MODEL` in `ollama-python-app.py` to use another.

## Project Structure

```
clients/ollama-py/
├── ollama-python-app.py   Main application — OllamaAgent, chat loop, tool execution
├── mcp_client.py          MCP client wrapper (connect, list tools, execute)
